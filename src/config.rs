...
pub fn cargar_configuracion() -> EmisorConfig {
    ...
    if !path.exists()
        ...
        && let Ok(contenido) = fs::read_to_string("config.json")
        && let Ok(config) = serde_json::from_str::<EmisorConfig>(&contenido)
    {
    ...
    if path.exists()
        && let Ok(contenido) = fs::read_to_string(&path)
        && let Ok(config) = serde_json::from_str::<EmisorConfig>(&contenido)
    {
        return config;
    }
...
pub fn guardar_configuracion(config: &EmisorConfig) -> Result<(), String> {
    ...
    fs::write(&path, json).map_err(|e| format!("Error guardando '{}': {}", path.display(), e))?;
    Ok(())
}
