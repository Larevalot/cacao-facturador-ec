...
pub fn construir_xml_factura(
    ...
    for (_, _, val) in resumen_iva.values() {
        total_iva += val;
    }
    ...
    if let Some(gr) = &request.guia_remision {
        let trimmed = gr.trim();
        if !trimmed.is_empty() && trimmed != "S/N" {
            xml.push_str(&format!(
    ...
    xml.push_str(&format!(
        "    <totalSinImpuestos>{:.2}</totalSinImpuestos>\n",
        total_sin_impuestos
    ));
    xml.push_str(&format!(
        "    <totalDescuento>{:.2}</totalDescuento>\n",
        total_descuento
    ));
    ...
    for d in &request.detalles {
        ...
        xml.push_str(&format!(
            "      <precioUnitario>{:.2}</precioUnitario>\n",
            d.precio_unitario
        ));
        xml.push_str(&format!(
        ...
        xml.push_str(&format!(
            "      <precioTotalSinImpuesto>{:.2}</precioTotalSinImpuesto>\n",
            d.subtotal()
        ));
    ...
    for p in &request.formas_pago {
            ...
            && plazo > 0
        {
            ...
            campos_adicionales.push((
                ...
                escape_xml(&format!(
                    "{}: {} {} (${:.2})",
                    nombre_fp, plazo, ut_label, p.total
                )),
            ));
        }
    }
...
/// Convierte entidades XML escapadas (&lt;, &gt;, &quot;, &apos;, &amp;) en caracteres normales.
